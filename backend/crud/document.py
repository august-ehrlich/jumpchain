...
async def get_document(db: AsyncSession, document_id: int):
    stmt = select(Document).options(
        selectinload(Document.categories).selectinload(TraitCategory.traits)
...
async def create_document(db: AsyncSession, document: DocumentCreate) -> Document:
    ...
    # Fetch the complete document hierarchy to return
    stmt = select(Document).options(
        selectinload(Document.categories).selectinload(TraitCategory.traits)
...
async def update_document(db: AsyncSession, document_id: int, document_update: DocumentUpdate):
    ...
    # Replaced bypass fields with "rules"
    for key in ["title", "choice_points", "summary", "has_random_age", "age_roll_min", "age_roll_max", "rules"]:

...
import { Document, TraitCategory, Trait } from '../../types/document';
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-hidden flex flex-col mt-4">
          <Tabs defaultValue="basics" className="flex-1 flex flex-col overflow-hidden">
            <TabsList className="w-full flex flex-wrap h-auto justify-start">
              <TabsTrigger value="basics" className="flex-1 min-w-[100px]">Basics & Settings</TabsTrigger>
              {formData.categories.map(cat => (
                <TabsTrigger key={cat.id} value={cat.id.toString()} className="flex-1 min-w-[100px]">{cat.name || "Unnamed"}</TabsTrigger>
              ))}
            </TabsList>
            ...
            <div className="flex-1 overflow-hidden py-4 flex flex-col">
              <TabsContent value="basics" className="space-y-4 m-0 overflow-y-auto h-full pr-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" required value={formData.choice_points} onChange={(e) => setFormData({ ...formData, choice_points: parseInt(e.target.value) || 0 })} />
                </div>
                <div className="space-y-2">
                  <Label>Summary</Label>
                  <Textarea required rows={4} value={formData.summary} onChange={(e) => setFormData({ ...formData, summary: e.target.value })} />
                </div>
              ...
              {formData.categories.map((cat, idx) => (
                <TabsContent key={cat.id} value={cat.id.toString()} className="m-0 h-full overflow-y-auto pr-2">
                  <TraitEditor 
                    traitName={cat.name} 
                    items={cat.traits} 
                    onChange={(newTraits) => updateCategoryTraits(idx, newTraits)} 

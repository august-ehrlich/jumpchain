...
export default function DocumentListView() {
  ...
  useEffect(() => {
    async function load() {
      try { setDocuments(await documentApi.getAll()); } 
      finally { setLoading(false); }
    }
    load();
  }, []);

  if (loading) return <div className="p-8 text-muted-foreground">Loading documents...</div>;
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        {documents.length === 0 && <p className="text-muted-foreground col-span-full">No documents found. Create one!</p>}
      </div>
